...
public:
    ...
    pair<int, int> help(TreeNode * root){
        ...
        pair<int, int> ret;
        ret.second = max({ root->val, root->val + left.second, root->val + right.second });
        ret.first = max(ret.second, root->val + left.second + right.second);
        if (root->left)
...
int main()
{
    {
        TreeNode n1(1);
        cout << Solution3().maxPathSum(&n1) << endl;
    }{
