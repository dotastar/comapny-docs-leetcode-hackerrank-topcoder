...
1）if di < dx，由于S(di)和S(dx)增加相同幅度，故最大超时还是在dx点；
2）else，S(di)及以后的S(d)都增加了，S(dx)未变，这时要重新计算最大超时点；
...
代码实现的一些细节：
1）由于1<=d<=kMaxD，所以实际上S(d) = sum[d-1]；
2）对于I(d)，情况稍微复杂：
	2.1）由于使用的反向BIT，故I(d)对应的BIT索引是(kMaxD + 1 - d)；
	2.2）由于1<=d<=kMaxD，故BIT索引(kMaxD + 1 - d)对应index[(kMaxD + 1 - d) - 1] = index[kMaxD - d]；
	2.3）根据实际意义，初始I(d)=d（从d开始的最大超时点，默认为d），而index全部初始化为0，故有一个转换关系：
		I(d) = index[kMaxD - d] + d
